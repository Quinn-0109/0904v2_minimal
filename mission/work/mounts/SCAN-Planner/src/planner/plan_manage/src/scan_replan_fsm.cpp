...
namespace scan_planner
{
  ...
  void SCANReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/manual_goal_use_current_height", manual_goal_use_current_height_, false);
  ...
  void SCANReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    bool success = false;
    // A manual goal used to be pinned to the height of the very first odom
    // sample.  That is correct for the original single-floor demo, but makes
    // every F2/F3 goal point back at F1 after a physical stair transition.
    // Capture the current body height at each goal when the multi-floor mode
    // is explicitly enabled.  The default preserves upstream behaviour.
    const double goal_height =
        manual_goal_use_current_height_ ? odom_pos_(2) : rviz_goal_height_;
    end_pt_ << msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, goal_height;
