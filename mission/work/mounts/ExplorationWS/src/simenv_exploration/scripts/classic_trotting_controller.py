...
from simenv_expl_devel_resolver import resolve_junior_ctrl  # noqa: E402
...
def main():
    workspace = os.path.realpath(os.environ.get(
        "SIMENV_EXPLORATION_WORKSPACE",
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../.."),
    ))
    # This mount carries no unitree_guide build of its own; the bundle keeps
    # one validated controller (see simenv_expl_devel_resolver docstring).
    # Preference order: SIMENV_EXPL_DEVEL_SPACE (explicit, set by the bundle
    # runner), then legacy mount-local devel/devel_0803b layouts.
