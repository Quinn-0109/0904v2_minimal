...
public:
    ...
    void load_policy();
    void policyRequestCallback(const std_msgs::String::ConstPtr& message);
...
private:
    ...
    bool loadPendingPolicyOnEntry();
    std::shared_ptr<torch::jit::script::Module> cachedPolicy(
        const std::string& policyPath, bool& cacheHit);
    void preloadConfiguredPolicies();
    torch::DeviceType inferenceDeviceForPolicy(
    ...
    std::unordered_map<std::string,
        std::shared_ptr<torch::jit::script::Module>> _policyCache;
    enum Color {
        STOP,
        RUNNING,
        OVER,
    ...
    long long _takeoverStartUs = 0;
    long long _stableSinceUs = 0;
    // FixedStand and the learned policy use different nominal joint poses.
    // Blend actual state into the policy target before evaluating a stationary
    // zero-command hold; applying the first policy action at full strength can
    // tip the simulated A1 even when cmd_vel is zero.
    float _blendDurationSec = 2.5f;
    float _zeroHoldSec = 3.0f;
    // A one-shot stair-landing re-entry, or a verified level-floor plane
    // reload, may use a shorter externally armed profile.  Normal startup
    // and unarmed reloads keep the defaults above; active values reset at
    // each FSM boundary/reload.
    float _activeBlendDurationSec = 2.5f;
    float _activeZeroHoldSec = 3.0f;
    float _takeoverFailureGraceSec = 2.0f;
    std::atomic_bool _takeoverFailed{false};
    std::atomic_bool _locomotionReady{false};
    // Plane-policy-only attitude guard.  Stair ascent intentionally carries a
    // larger pitch and is excluded; flat-floor commands are smoothly removed
    // before the global 60-degree FSM fall threshold can be reached.
    float _planeTiltGuardWarningRad = 0.20f;
    float _planeTiltGuardStopRad = 0.32f;
    float _planeTiltGuardReleaseRad = 0.14f;
    bool _planeTiltGuardHold = false;
    float _planeTiltGuardScale = 1.0f;
    ros::Publisher _readyPub;
    ros::Publisher _statusPub;
    ros::Subscriber _policyRequestSub;
    std::mutex _policyMutex;
    std::string _activePolicyPath;
    std::string _pendingPolicyPath;
    std::string _loadingPolicyPath;
    std::atomic_bool _policyReloadRequested{false};
};
