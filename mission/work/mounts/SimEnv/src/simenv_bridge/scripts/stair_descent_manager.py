...
下行返回一楼。每段下梯(与上梯相反,先 flight_b 后 flight_a):
  ...
  DESCENT_ENTRY_GUIDE(仅段 1:F3 走廊口 → landing 侧点 → landing 中心)
    ...
    → STAIR_DESCENT_FLIGHT_A(沿 -y 下行,drop ≥2.45 m 且 y ≤1.75)
    → 到达下一层 landing
...
class StairDescent:
    def __init__(self):
        ...
        # A hot plane->stair TorchScript reload can issue one mismatched gait
        # sample before the new observation history settles.  Freeze a fresh,
        # low-velocity joint snapshot in FixedStand, queue the policy while RL
        # is inactive, then make one bounded fast RL re-entry instead.
        ...
        self.policy_switch_fast_fixed_stand=bool(rospy.get_param(
            '~policy_switch_fast_fixed_stand', False))
        ...
        self.truth_landing_size=None        # landing 碰撞体 x/y 尺寸
        self.truth_final_landing_surface_z=None
        ...
        # 0.80 m/s remains the centred cruise speed, but commanding it before
        # the stair gait has loaded its first tread can occasionally overrun
        # the contact cycle. Shape each flight by its own vertical progress:
        # a short 0.55 m/s entry, a compact ramp, and a 0.60 m/s landing cap.
        ...
        self.flight_tilt_slowdown=max(0.0, float(rospy.get_param(
            '~truth_descent_flight_tilt_slowdown_rad', .45)))
        self.flight_tilt_speed=max(.30, float(rospy.get_param(
            '~truth_descent_flight_tilt_speed_mps', .50)))
        ...
        # Keep the requested high speed on a centred flight, but shed speed
        # before lateral drift reaches the hard guard.  A full-mission 0.80
        # m/s run rolled at the last tread after centre error grew to 0.47 m;
        # the already-centred isolated run never needed this branch.
        self.flight_center_correction_error=max(0.0, float(rospy.get_param(
            '~truth_descent_flight_center_correction_error_m', .28)))
        self.flight_center_correction_speed=max(.30, float(rospy.get_param(
            '~truth_descent_flight_center_correction_speed_mps', .55)))
        self.flight_center_correction_yaw_rate=max(0.0, float(rospy.get_param(
            '~truth_descent_flight_center_correction_yaw_rate_rps', .14)))
        ...
        # Once the half-turn reaches its target, switch permanently to the
        # shortest signed yaw error while lateral alignment finishes. Keeping
        # the initial direction latched used to command a full extra turn when
        # x converged one tick after yaw.
        self.landing_heading_captured=False
        ...
        # tf_goal15(2026-08-24):F2 平台北缘实测 y≈2.05，旧对称上界
        # y_keep+0.35=2.40 会允许转体弧线漂到 y=2.14 后踩上 tread；随后
        # pitch -0.81 / roll -0.97、z 2.96→2.75，转体永久冻结。留 0.10m
        # 制动余量，在 y=1.95 先停转并回 align 南移。
        configured_y_high=float(rospy.get_param(
            '~truth_descent_segment_turn_y_safe_high_m', 1.95))
        self.segment_turn_y_high=max(
            self.segment_turn_y_low,
            min(self.segment_turn_y_keep+self.segment_turn_y_band,
                configured_y_high))
        # align 是 stair policy 的 body 侧移；小于实测约 0.24m/s 的比例
        # 命令不会落步。旧 x_err=0.251→vx=0.226 在安全护栏回中时冻结，
        # 被误当作 yaw stall 后反复 recovery。使用 0.30 的有效最低率。
        self.segment_turn_alignment_minimum_speed=float(rospy.get_param(
            '~truth_descent_segment_turn_alignment_minimum_speed_mps', .30))
        self.segment_turn_alignment_progress_timeout=max(
            .5, float(rospy.get_param(
                '~truth_descent_segment_turn_alignment_progress_timeout_sec',
                3.0)))
        self.segment_turn_alignment_progress_min=max(0.0, float(
            rospy.get_param(
                '~truth_descent_segment_turn_alignment_progress_min_m',
                .025)))
        ...
        # tf_goal14 fix: directed turn-progress watchdog (active only while
        # segment_turn_stage == 'turn'). tf_goal14 实证:转体起步后
        # abs(turn_error) 数秒无有效下降,西/南漂移直至翻倒(roll≈-1.43),
        # 45s 终端超时只是后果。看门狗跟踪锁存方向下无界 turn_error 的
        # 历史最优绝对值(非相邻 wrapped-yaw 微差);仅当最优值改善 ≥
        # min_improvement 才刷新截止时间,错误方向/抖动不刷新。
        self.segment_turn_progress_timeout=float(rospy.get_param(
            '~truth_descent_segment_turn_progress_timeout_sec', 2.8))
        self.segment_turn_progress_min_improvement_rad=float(rospy.get_param(
            '~truth_descent_segment_turn_progress_min_improvement_rad', .11))
        # tf_goal14 fix: 对称 x 安全护栏。成功运行(tf_goal8)转体 x 偏移
        # ~0.20m 内;失败运行越过 ~0.25m 后持续西漂直至翻倒。旧的仅东向
        # drift_limit(settle)分支对未完成航向无效(settle 底部立即切回
        # turn),由本护栏取代:x 超出 x_keep±tolerance 或 y 出安全带 →
        # 立即零命令并回 align 重对中(同 tick 返回,不再发转体命令)。
        self.segment_turn_x_safe_tolerance=float(rospy.get_param(
            '~truth_descent_segment_turn_x_safe_tolerance_m', .25))
        ...
        # fixY(2026-08-24):east 段 x 比例命令低于楼梯策略侧移有效阈值
        # (~0.24 m/s,保守估计)。RUN 实证:x_err≈0.21 → 比例 vx≈0.19
        # (position_gain 0.90),在 yaw≈1.65 处旋转换到 body 分量约
        # vx=-0.025 / vy=-0.186(旋转保模长 ≈0.188):主导的 body 侧移
        # 分量 ~0.186 仍低于阈值 → 步态无响应,x 冻结 24s →
        # EAST_ALIGN_TIMEOUT。x 仍在容差外时强制 stair-policy-effective
        # 最低东西向率 0.30(阈值上留余量,仍低于 landing_recenter_speed
        # 0.40 cap)。y>1.95 守卫在其后无条件置 vx=0,本规则不会重新
        # 启用东移。
        self.east_align_minimum_x_speed=float(rospy.get_param(
            '~truth_descent_east_align_minimum_x_speed_mps', .30))
        # fixZ(2026-08-24):east 段相对 tread 进入检测 —— 绝对阈值 2.75
        # 太迟(RUN 实证 z 2.937→2.872 已上 tread 且 roll 0.345,继续对齐
        # 到 x=-3.12 侧翻躺平)。EAST_ALIGN 起始捕获参考 z,south 平台
        # 噪声 ≤~0.02,上 tread 跳变 ~0.087 → 0.05 阈值区分二者。
        self.east_align_tread_drop=float(rospy.get_param(
            '~truth_descent_east_align_tread_drop_m', .05))
        # fixZ(2026-08-24):truth 姿态预终态守卫 —— 与 Unitree 安全条件
        # rotMat(2,2)<0.5 同源的 upright-z=cos(roll)·cos(pitch),取 0.55
        # 在 Unitree 被动化前抢先终态(不修改 Unitree 阈值)。倾覆后 z
        # 不再骤降,旧 z-only 坠落守卫不触发,manager 会躺平假 stall
        # 恢复 100+s;健康楼梯姿态 pitch 至 ~0.61 时 upright-z≈0.82,
        # 远高于门槛。
        self.minimum_upright_z=float(rospy.get_param(
            '~truth_descent_minimum_upright_z', .55))
        ...
        self.flight_a_end_y=float(rospy.get_param(
            '~truth_descent_flight_a_end_y', 1.75))
        # 最后一段不能在最后一级踏步上直接写成功终态。机器人中心进入
        # landing 后持续零命令，要求位置、姿态和真实机身高度连续稳定，
        # 再交给平地 policy。实际 stair-RL 平地站姿约 0.31 m，故高度
        # 下限按 landing 表面 +0.20 m，而不是出生时的 0.60 m 名义高度。
        self.final_landing_settle_seconds=float(rospy.get_param(
            '~truth_descent_final_landing_settle_seconds', 3.0))
        self.final_landing_timeout=float(rospy.get_param(
            '~truth_descent_final_landing_timeout_sec', 20.0))
        self.final_landing_min_upright_z=float(rospy.get_param(
            '~truth_descent_final_landing_minimum_upright_z', .75))
        self.final_landing_max_tilt=float(rospy.get_param(
            '~truth_descent_final_landing_max_tilt_rad', .45))
        self.final_landing_min_body_height=float(rospy.get_param(
            '~truth_descent_final_landing_min_body_height_m', .20))
        self.final_landing_max_body_height=float(rospy.get_param(
            '~truth_descent_final_landing_max_body_height_m', 1.0))
        self.final_landing_started_at=None
        self.final_landing_good_since=None
        self.descent_start_z=None
        self.flight_started_at=None
        self.flight_start_z=None
        ...
        # tf_goal14 fix: directed-progress watchdog 状态(turn 阶段专用)
        self.segment_turn_progress_best_abs=None
        self.segment_turn_progress_deadline=None
        self.segment_turn_alignment_best_error=None
        self.segment_turn_alignment_deadline=None
        self.segment=0
    ...
    def on_policy_status(self, message):
        if ('policy_reloaded:' in message.data and
                os.path.basename(self.policy) not in message.data and
                self.phase == 'WAIT_F3'):
        ...
        elif 'policy_reload_failed:' in message.data and self.phase in (
                'STAIR_DESCENT_POLICY_LOADING',
    ...
    def on_joint_states(self, message):
        ...
        velocity_rms=(
            math.sqrt(sum(value*value for value in velocities) /
        ...
        try:
            ...
            if index < len(message.twist):
                ...
                self.truth_twist=(
                    ...
                    float(twist.linear.z), float(twist.angular.z))
    ...
    def _publish_fixed_stand(self):
        ...
        message.axes=[0.0]*8
        message.buttons=[0]*12
        message.buttons[1]=1  # Unitree L2_A fixed stand
        self.joy.publish(message)
    ...
    def _capture_stand_target(self):
        """Freeze one recent low-speed plane-gait sample in FSM order."""
        with self._joint_lock:
        ...
        if (not names or positions is None or received is None or
                time.monotonic()-received > 2.0):
            return False
        if rms is not None and rms > self.policy_switch_joint_velocity_rms:
            return False
        by_name=dict(zip(names, positions))
        try:
            target=[float(by_name[name]) for name in self._FSM_JOINT_ORDER]
        except (KeyError, TypeError, ValueError):
            return False
        if len(target) != 12 or not all(math.isfinite(value) for value in target):
            return False
        try:
            rospy.set_param(self._stand_target_param, target)
        except Exception as error:
            rospy.logerr('Descent policy boundary could not set stand target: %s',
                         error)
            return False
        self._stand_target_applied=True
        rospy.loginfo('Descent policy boundary captured joint RMS %s.',
                      'unknown' if rms is None else '%.3f' % rms)
        return True
    ...
    def _clear_stand_target(self):
        ...
        try:
            ...
            rospy.logerr('Descent policy boundary could not clear %s: %s',
    ...
    def _arm_fast_takeover(self):
        ...
        try:
            for name in (
                ...
                rospy.set_param(name, self._fast_takeover_params[name])
        except Exception as error:
            rospy.logerr('Descent could not arm fast stair takeover: %s', error)
    ...
    def _clear_fast_takeover(self):
        if not self._fast_takeover_profile_owned:
            return True
        ok=True
        for name in self._fast_takeover_params:
            try:
                ...
                ok=False
                rospy.logerr('Descent could not clear takeover parameter %s: %s',
                             name, error)
        if ok:
            self._fast_takeover_profile_owned=False
        return ok
    ...
    def _arm_fast_fixed_stand(self):
        """Arm a one-shot readiness profile for the captured joint pose."""
        ...
        try:
            for name in (
                ...
                rospy.set_param(name, self._fast_fixed_stand_params[name])
        except Exception as error:
            rospy.logerr('Descent could not arm fast FixedStand: %s', error)
    ...
    def _clear_fast_fixed_stand(self):
        if not self._fast_fixed_stand_profile_owned:
            return True
        ok=True
        for name in self._fast_fixed_stand_params:
            try:
                if rospy.has_param(name):
                    rospy.delete_param(name)
            except KeyError:
                ...
                ok=False
                rospy.logerr(
                    'Descent could not clear FixedStand parameter %s: %s',
                    name, error)
        if ok:
            self._fast_fixed_stand_profile_owned=False
        return ok
    ...
    @staticmethod
    def _limit_world_linear(vx, vy, cap):
        """Direction-preserving 2-D limit with a nonnegative speed cap."""
        effective_cap=max(0.0, cap)
        speed=math.hypot(vx, vy)
        if effective_cap <= 0.0:
            return 0.0, 0.0
        if speed > effective_cap:
            scale=effective_cap/speed
    ...
    def _load_landing_center(self):
        ...
        try:
            ...
            if len(size_values) >= 2:
                self.truth_landing_size=(float(size_values[0]),
                                         float(size_values[1]))
            final_name='stair_floor_landing_floor_{}'.format(
                self.end_floor_number-1)
            final_link=next(item for item in root.iter('link')
                            if item.get('name') == final_name)
            final_pose=(final_link.findtext('pose') or '').split()
            final_size=(final_link.findtext(
                'collision/geometry/box/size') or '').split()
            if len(final_pose) >= 3 and len(final_size) >= 3:
                self.truth_final_landing_surface_z=(
                    float(final_pose[2])+float(final_size[2])/2.0)
    ...
    def _record_trace(self, force=False):
        ...
        if self.phase in ('STAIR_DESCENT_FLIGHT_A',
                          'STAIR_DESCENT_FLIGHT_B'):
            command_speed=getattr(self, 'flight_command_axis_speed', None)
            scheduled_speed=getattr(self, 'flight_scheduled_speed', None)
            center_error=getattr(self, 'flight_command_center_error', None)
            heading_error=getattr(self, 'flight_command_heading_error', None)
            item.update({
                'command_axis_speed_mps': (
                    round(command_speed, 3)
                    if command_speed is not None else None),
                'scheduled_speed_mps': (
                    round(scheduled_speed, 3)
                    if scheduled_speed is not None else None),
                'center_error_m': (
                    round(center_error, 3)
                    if center_error is not None else None),
                'heading_error_rad': (
                    round(heading_error, 3)
                    if heading_error is not None else None),
                'center_correction_active': bool(getattr(
                    self, 'flight_center_correction_active', False)),
                'heading_correction_active': bool(getattr(
                    self, 'flight_heading_correction_active', False)),
                'tilt_speed_cap_active': bool(getattr(
                    self, 'flight_tilt_cap_active', False)),
                'severe_tilt_speed_cap_active': bool(getattr(
                    self, 'flight_severe_tilt_cap_active', False)),
            })
    ...
    def _begin_segment(self):
        ...
        if self.segment == 0 and self.truth_entry_guide and \
            ...
            self.segment_turn_direction=None
            self.segment_turn_turn_started_at=None
            self._reset_segment_turn_progress()
            self._reset_segment_turn_alignment_progress()
    ...
    def _enter_policy_loading(self):
        """Enter stair RL, using a FixedStand boundary for a real swap."""
        self.cmd.publish(Twist())
        if self.policy_loaded:
            ...
            rospy.loginfo('Descent reusing the already active stair policy.')
            return
    ...
    def _policy_switch_settle_tick(self):
        ...
        if self._policy_switch_body_stable():
            ...
            self.policy_switch_stable_since=None
        stable_elapsed=(
            now-self.policy_switch_stable_since
            if self.policy_switch_stable_since is not None else 0.0)
        if (stable_elapsed >= self.policy_switch_settle_seconds and
            ...
            self.locomotion_ready_at=None
            self._publish_fixed_stand()
            rospy.loginfo('Descent plane gait settled; entering FixedStand '
                          'before the stair-policy load.')
            self._record_trace(force=True)
            return
        if (self.policy_switch_settle_started is not None and
                now-self.policy_switch_settle_started >=
    ...
    def _policy_switch_stand_tick(self):
        ...
        if fresh_ready:
            ...
            self.state.publish(String(data=self.phase))
            rospy.loginfo('Descent FixedStand is ready; stair policy queued.')
            self._record_trace(force=True)
            return
        if (self.policy_switch_stand_started is not None and
                now-self.policy_switch_stand_started >=
    ...
    def _policy_switch_queued_tick(self):
        ...
        self.cmd.publish(Twist())
        self._publish_fixed_stand()
        if (self.policy_switch_queued_at is None or
                now-self.policy_switch_queued_at <
                self.policy_switch_queue_hold_seconds):
            self._record_trace()
            return
        ...
        self.state.publish(String(data=self.phase))
        self.hold_rl()
        rospy.loginfo('Descent stair policy queued under FixedStand; '
                      'requesting fresh stair-RL entry.')
        self._record_trace(force=True)
    ...
    def _policy_switch_reentry_tick(self):
        ...
        self.hold_rl()
        fresh=(
            self.policy_loaded and self.locomotion_ready and
            self.locomotion_ready_at is not None and
            self.policy_switch_reentry_started is not None and
        ...
        if fresh:
            ...
            rospy.loginfo('Descent stair RL fresh readiness confirmed; '
                          'warming up %.1f s.', self.policy_warmup_seconds)
            self._record_trace(force=True)
            return
        if (self.policy_switch_reentry_started is not None and
                now-self.policy_switch_reentry_started >=
    ...
    def _begin_east_align(self):
        ...
        # fixZ(2026-08-24):EAST_ALIGN 起始参考 z(east 段相对 tread 跳变
        # 检测的基准;south 平台噪声 ≤~0.02 不触发)。
        self.east_align_reference_z=(self.truth_pose[2]
    ...
    def _east_align_tick(self):
        ...
        if self.truth_pose is None:
            return
        now=time.monotonic()
        # fixZ(2026-08-24):姿态预终态守卫先于一切(含超时/命令)——
        # 倾覆后继续发布侧移命令只会加速侧翻;upright-z 同源 Unitree
        # rotMat(2,2),0.55 门在 Unitree 被动化(0.5)前抢先终态。
        if self._tipped_over():
            self._terminal('STAIR_DESCENT_EAST_ALIGN_ATTITUDE_LOST',
                           'stair_descent_east_align_attitude_lost',
                           error=True)
            return
        ...
        if self.truth_pose[2] <= self.east_align_tread_z:
            rospy.loginfo('East-align entered flight-b tread (z=%.3f); '
                          'handing off to segment-2 flight-b.',
                          self.truth_pose[2])
        ...
        if self.east_align_stage == 'south':
            ...
            # fixZ(2026-08-24):相对 tread 进入早交接 —— 参考 z 比当前高
            # ≥0.05(south 噪声 ≤~0.02,tread 跳变 ~0.087)= 已踩上
            # flight_b 上层 tread;立即零命令+保持 RL,同 tick 交接
            # FLIGHT_B 返回,不等绝对 2.75 兜底,也不发布下一条侧移命令。
            # 仅 east 段:south 南移本身允许下 1-2 级台阶,不允许相对判据。
            east_tread_drop=(self.east_align_reference_z-self.truth_pose[2]
                             if self.east_align_reference_z is not None
                             else None)
            # A mathematically exact 0.05 can be represented as
            # 0.04999999999999982; the tiny epsilon only removes that numeric
            # boundary artefact and is negligible relative to pose noise.
            if (east_tread_drop is not None and
                    east_tread_drop+1e-9 >= self.east_align_tread_drop):
                self.cmd.publish(Twist())
                self.hold_rl()
                rospy.logwarn(
                    'East-align stepped onto flight-b tread '
                    '(z=%.3f, drop=%.3f); zero command and handing off '
                    'to segment-2 flight-b.', self.truth_pose[2],
                    east_tread_drop)
                self._begin_flight('b')
                return
            ...
            # fixY(2026-08-24):x 容差外小比例命令死区提升 —— x_err≈0.21
            # 时比例 vx≈0.19(gain 0.90),旋转到 body(yaw≈1.65)后主导
            # 侧移分量 ~0.186 低于楼梯策略侧移有效阈值 ~0.24 → 步态无
            # 响应,x 冻结到 30s 超时。x 仍在容差外时提升到有效最低率
            # (方向取 x_err 符号,gain=0 的外部覆盖也不会反方向);
            # 有效最低率先夹到 [0, landing_recenter_speed],运维把最低率
            # 配到 cap 之上时也绝不超过既有 cap。y>1.95 守卫在下方仍
            # 无条件置 vx=0,优先级不变。
            effective_minimum=max(
                0.0, min(self.east_align_minimum_x_speed,
                         self.landing_recenter_speed))
            if (abs(x_err) > self.segment_turn_position_tolerance and
                    abs(vx) < effective_minimum):
                vx=math.copysign(effective_minimum, x_err)
        ...
        if abs(yaw_err) < 0.08:
            wz=0.0
        # fixZ(2026-08-24):合成世界线性命令限幅 —— vx/vy 各自 cap 0.40
        # 但 (0.40,-0.40) 模长 0.566 过大(RUN 实证侧翻前一刻的命令)。
        # 在 y>1.95 守卫选定 (vx,vy) 之后、发布之前等比缩到模长 ≤
        # landing_recenter_speed(保持方向/比例);y 守卫已把 vx 置 0,
        # 缩放不会再启用 x。
        vx, vy=self._limit_world_linear(
            vx, vy, self.landing_recenter_speed)
    ...
    def tick(self, _):
        desired_pause=self.phase in (
            ...
            'STAIR_DESCENT_FINAL_LANDING_SETTLE')
        ...
        if self.phase == 'STAIR_DESCENT_FLIGHT_A':
            ...
            return
        if self.phase == 'STAIR_DESCENT_FINAL_LANDING_SETTLE':
            self._final_landing_settle_tick()
            return
    ...
    def _pre_align_tick(self):
        ...
        if abs(yaw_rate) < self.landing_minimum_yaw_rate:
            yaw_rate=math.copysign(self.landing_minimum_yaw_rate, error)
    ...
    def _flight_control(self, which):
        ...
        发出(世界系,投影后 body 恒为纯轴),由进/出梯与姿态调度器选择
        0.55→0.80→0.60 m/s 的轴向速度,再由 yaw
        ...
        self.flight_command_axis_speed=axis_speed
        self.flight_command_center_error=center_error
        self.flight_command_heading_error=heading_error
        self.flight_center_correction_active=correction_active
        self.flight_heading_correction_active=heading_correction_active
    ...
    def _scheduled_flight_speed(self):
        """Return the per-tread speed cap while preserving 0.80 m/s cruise."""
        ...
        if self.truth_attitude is not None:
            ...
            if tilt >= self.flight_tilt_slowdown:
        ...
        if (self.flight_severe_tilt_until is not None and
            ...
            speed=min(speed, self.flight_severe_tilt_speed)
              ...
              now >= self.flight_severe_tilt_until):
            self.flight_severe_tilt_until=None
        self.flight_scheduled_speed=speed
        self.flight_tilt_cap_active=bool(
            self.flight_tilt_slow_until is not None and
            now < self.flight_tilt_slow_until)
        self.flight_severe_tilt_cap_active=bool(
            self.flight_severe_tilt_until is not None and
            now < self.flight_severe_tilt_until)
        return speed

    def _tipped_over(self):
        """fixZ(2026-08-24):truth 姿态倾覆判据 —— upright-z=
        cos(roll)·cos(pitch)(与 Unitree rotMat(2,2) 同源)低于门槛
        (默认 0.55,略高于 Unitree 硬门 0.5,提前于被动化终态)。"""
        if self.truth_attitude is None:
            return False
        upright_z=(math.cos(self.truth_attitude[0]) *
                   math.cos(self.truth_attitude[1]))
        return upright_z < self.minimum_upright_z

    def _flight_tick(self, which):
        if self.truth_pose is None or self.descent_start_z is None:
            return
        now=time.monotonic()
        # fixZ(2026-08-24):姿态预终态守卫先于坠落/回升/stall 恢复 ——
        # 躺平后 z 不再骤降,旧 z-only 守卫不触发,stall 恢复会空转
        # 100+s(RUN 实证 roll≈π 仍持续 backoff)。
        if self._tipped_over():
            self._terminal('STAIR_DESCENT_FLIGHT_ATTITUDE_LOST',
                           'stair_descent_flight_attitude_lost', error=True)
            return
        ...
        if which == 'b':
            ...
            if drop >= self.flight_a_descent_drop and \
                ...
                if last_segment and self.end_floor_number != 2:
                    self.final_landing_started_at=now
                    self.final_landing_good_since=None
                    self.phase='STAIR_DESCENT_FINAL_LANDING_SETTLE'
                    self.state.publish(String(data=self.phase))
                    self.cmd.publish(Twist())
                    self.hold_rl()
                    rospy.loginfo(
                        'Final flight complete: drop=%.2f m y=%.2f; '
                        'holding on F1 landing for a stable terminal pose.',
                        drop, self.truth_pose[1])
                    self._record_trace(force=True)
                else:
        ...
        if (self.flight_pause_until is not None or
            ...
            self.flight_stall_since=None
            self.flight_stall_window_start_z=None
        if drop >= self.stall_arm_drop and \
            ...
            if now-self.flight_stall_since >= self.stall_seconds:
                ...
                self.flight_stall_since=now
                self.flight_stall_window_start_z=self.truth_pose[2]
                if z_drop < self.stall_z_progress:
    ...
    def _final_landing_settle_tick(self):
        """Require a continuous, physical F1 landing hold before success."""
        if self.truth_pose is None:
            return
        now=time.monotonic()
        self.cmd.publish(Twist())
        self.hold_rl()
        if self._tipped_over():
            self._terminal('STAIR_DESCENT_FINAL_LANDING_ATTITUDE_LOST',
                           'stair_descent_final_landing_attitude_lost',
                           error=True)
            return

        center=self.truth_landing_center or (-3.25, 1.55)
        size=self.truth_landing_size or (3.02, 1.00)
        on_landing=(abs(self.truth_pose[0]-center[0]) <= size[0]/2.0 and
                    abs(self.truth_pose[1]-center[1]) <= size[1]/2.0)
        roll, pitch=(self.truth_attitude if self.truth_attitude is not None
                     else (float('inf'), float('inf')))
        upright_z=(math.cos(roll)*math.cos(pitch)
                   if math.isfinite(roll) and math.isfinite(pitch) else -1.0)
        surface_z=(self.truth_final_landing_surface_z
                   if self.truth_final_landing_surface_z is not None else 0.0)
        body_height=self.truth_pose[2]-surface_z
        stable=(on_landing and
                upright_z >= self.final_landing_min_upright_z and
                abs(roll) <= self.final_landing_max_tilt and
                abs(pitch) <= self.final_landing_max_tilt and
                self.final_landing_min_body_height <= body_height <=
                self.final_landing_max_body_height)
        if stable:
            if self.final_landing_good_since is None:
                self.final_landing_good_since=now
            if now-self.final_landing_good_since >= \
                    self.final_landing_settle_seconds:
                rospy.loginfo(
                    'F1 landing stable for %.1f s: body height %.3f m, '
                    'upright-z %.3f.', self.final_landing_settle_seconds,
                    body_height, upright_z)
                self._segment_complete()
                return
        else:
            self.final_landing_good_since=None

        if (self.final_landing_started_at is not None and
                now-self.final_landing_started_at >=
                self.final_landing_timeout):
            self._terminal('STAIR_DESCENT_FINAL_LANDING_UNSTABLE',
                           'stair_descent_final_landing_unstable', error=True)
            return
        self._record_trace()
    ...
    def _landing_turn_tick(self):
        ...
        if not self.landing_heading_captured:
            ...
            if abs(error) <= self.landing_heading_tolerance:
                self.landing_heading_captured=True
                turn_error=error
                rospy.loginfo(
                    'Descent landing heading captured at error %.3f rad; '
                    'using shortest-path hold while x aligns.', error)
        else:
            turn_error=error
    ...
    def _seed_segment_turn_progress(self, abs_error, now):
        """Seed the directed turn-progress watchdog at turn entry."""
        self.segment_turn_progress_best_abs=abs_error
        self.segment_turn_progress_deadline=(now+
            self.segment_turn_progress_timeout)

    def _reset_segment_turn_progress(self):
        """Clear the directed turn-progress watchdog state."""
        self.segment_turn_progress_best_abs=None
        self.segment_turn_progress_deadline=None

    def _reset_segment_turn_alignment_progress(self):
        """Clear the flat-landing position-progress watchdog state."""
        self.segment_turn_alignment_best_error=None
        self.segment_turn_alignment_deadline=None

    def _begin_segment_turn_recovery(self, now, reason):
        """One bounded replant/pause/east-backoff recovery for the segment
        turn (shared by z-drop, yaw-stall and directed-no-progress paths
        so they cannot diverge). Respects landing_slip_max_attempts."""
        if self.landing_slip_recoveries >= self.landing_slip_max_attempts:
            return False
        self.landing_slip_recoveries+=1
        self.landing_slip_active=True
        self.landing_slip_until=now+self.landing_slip_pause_seconds
        ...
        self._reset_segment_turn_alignment_progress()
        rospy.logwarn(
            'Descent segment-turn recovery (%s): re-planting gait '
            '(recovery %d/%d).', reason, self.landing_slip_recoveries,
            self.landing_slip_max_attempts)
        self._record_trace()
        return True
    ...
    def _segment_turn_tick(self):
        ...
        if self.landing_z0 is not None:
            z_drop=self.landing_z0-self.truth_pose[2]
            if (z_drop > self.landing_slip_drop and
                    self._begin_segment_turn_recovery(
                        now, 'z dropped %.2f m' % z_drop)):
                return
        ...
        #      缓慢同样累积 → 打滑恢复) ----
        # yaw stall 只在真正发布转向命令的 turn 阶段计时。align 阶段
        # 本来 wz=0，旧逻辑把正常的位置回中误计成 6s yaw stall，随后
        # 触发不必要的 replant/backoff 并重启整个转向窗口。
        if (self.segment_turn_stage == 'turn' and
                not (turn_done and position_ok)):
            dt=(now-self.landing_last_yaw_at
                if self.landing_last_yaw_at is not None else 0.0)
            # R31: 50Hz tick(dt≈0.02s)使 0.05 下界永不满足,stall 检测
            # 完全失效(R30 实证:align 侧移冻结 37s 无恢复→超时)。
            ...
            if (self.landing_yaw_stall_accum >=
                    self.landing_yaw_stall_seconds and
                    self._begin_segment_turn_recovery(
                        now, 'heading err %.3f rad stuck %.1f s' %
                        (error, self.landing_yaw_stall_seconds))):
                return
        else:
            self.landing_yaw_stall_accum=0.0
        ...
        if self.segment_turn_stage == 'align':
            ...
            if (abs(x_err) <= self.segment_turn_position_tolerance and
                ...
                # 安全护栏造成的短暂 align 不重启 2s 慢启动；否则每个
                # 小转向片段都回到 0.24rps，来不及取得有效进度。真正的
                # replant 会把该时间清空，仍会重新慢启动。
                if self.segment_turn_turn_started_at is None:
                    self.segment_turn_turn_started_at=now
                self._seed_segment_turn_progress(abs(turn_error), now)
                self._reset_segment_turn_alignment_progress()
                ...
                vx=max(-self.landing_recenter_speed,
                       min(self.landing_recenter_speed,
                           self.landing_position_gain*x_err))
                effective_x_min=max(
                    0.0, min(self.segment_turn_alignment_minimum_speed,
                             self.landing_recenter_speed))
                if (abs(x_err) > self.segment_turn_position_tolerance and
                        abs(vx) < effective_x_min):
                    vx=math.copysign(effective_x_min, x_err)
                vy=0.0
                y_boundary_error=None
                if self.truth_pose[1] < self.segment_turn_y_low:
                    y_boundary_error=(self.segment_turn_y_low-
                                      self.truth_pose[1])
                elif self.truth_pose[1] > self.segment_turn_y_high:
                    y_boundary_error=(self.segment_turn_y_high-
                                      self.truth_pose[1])
                align_error=math.hypot(
                    x_err, y_boundary_error or 0.0)
                if self.segment_turn_alignment_best_error is None:
                    self.segment_turn_alignment_best_error=align_error
                    self.segment_turn_alignment_deadline=(now+
                        self.segment_turn_alignment_progress_timeout)
                elif (self.segment_turn_alignment_best_error-align_error >=
                      self.segment_turn_alignment_progress_min):
                    self.segment_turn_alignment_best_error=align_error
                    self.segment_turn_alignment_deadline=(now+
                        self.segment_turn_alignment_progress_timeout)
                elif (self.segment_turn_alignment_deadline is not None and
                      now >= self.segment_turn_alignment_deadline and
                      self._begin_segment_turn_recovery(
                          now, 'landing alignment stalled at %.3f m error' %
                          align_error)):
                    return
                if y_boundary_error is not None:
                    vy=max(-self.segment_turn_side_speed,
                           min(self.segment_turn_side_speed,
                    ...
                    effective_y_min=max(
                        0.0, min(self.segment_turn_alignment_minimum_speed,
                                 ...
                                 self.landing_recenter_speed))
                    if abs(vy) < effective_y_min:
                        vy=math.copysign(effective_y_min,
                                         y_boundary_error)
                vx, vy=self._limit_world_linear(
                    vx, vy, self.landing_recenter_speed)
        ...
        if self.segment_turn_stage == 'turn':
            ...
            if turn_done:
            ...
            elif (abs(x_err) > self.segment_turn_x_safe_tolerance or
                    not (self.segment_turn_y_low <= self.truth_pose[1] <=
                         self.segment_turn_y_high)):
                # tf_goal14 fix: 对称安全位置护栏(取代旧仅东向
                # drift_limit→settle 分支:settle 对未完成航向无效,底部
                # 立即切回 turn,不会重对中)。x 超出 x_keep±安全容差
                # (成功 ~0.20m,失败越过 ~0.25m)或 y 出安全带 → 立即零
                # 命令 + 回 align 重对中,本 tick 返回(不落地发转体
                # 命令,避免同 tick 又切回 turn)。回 turn 时看门狗重新
                # 种子。
                self.cmd.publish(Twist())
                self.hold_rl()
                self.segment_turn_stage='align'
                ...
                self._reset_segment_turn_progress()
                rospy.logwarn(
                    'Segment turn left safe position (x=%.3f, y=%.3f); '
                    'zero command and re-centering before continuing turn.',
                    self.truth_pose[0], self.truth_pose[1])
                self._record_trace()
                return
            else:
                # tf_goal14 fix: directed-progress watchdog —— 跟踪锁存
                # 方向下无界 turn_error 的最优绝对值;仅当改善 ≥
                # min_improvement 刷新截止,错误方向/抖动不刷新。超时仍
                # 无有效进展 → 仍在直立时进入有界 replant 恢复,不等
                # 45s 终端超时。
                abs_error=abs(turn_error)
                if self.segment_turn_progress_best_abs is None:
                    self._seed_segment_turn_progress(abs_error, now)
                elif (self.segment_turn_progress_best_abs-abs_error >=
                        self.segment_turn_progress_min_improvement_rad):
                    self.segment_turn_progress_best_abs=abs_error
                    self.segment_turn_progress_deadline=(now+
                        self.segment_turn_progress_timeout)
                if (self.segment_turn_progress_deadline is not None and
                        now >= self.segment_turn_progress_deadline and
                        self._begin_segment_turn_recovery(
                            now, 'directed turn progress stalled at '
                            'abs(err)=%.3f rad' % abs_error)):
                    return
                ...
                if self.truth_pose[1] < self.segment_turn_y_low:
                    ...
                    vy+=max(-self.segment_turn_side_speed,
                            min(self.segment_turn_side_speed,
                                ...
                                (self.segment_turn_y_high-
                                 self.truth_pose[1])))
                # body lateral 与 world x/y hold 叠加曾达到约 0.57m/s。
                # 保持既有 0.40m/s 线速度包络，等比保留转向步态方向。
                vx, vy=self._limit_world_linear(
                    vx, vy, self.landing_recenter_speed)
                self._publish_truth_world_command(vx, vy, yaw_rate)
                self._record_trace()
        ...
        if not turn_done:
            # tf_goal14 fix: settle 对未完成航向不再有效停留 —— 回 turn
            # 必须重置/种子 directed-progress watchdog。
            self.segment_turn_stage='turn'
            self._seed_segment_turn_progress(abs(turn_error), now)
            return
        vx=max(-self.landing_recenter_speed,
        ...
        try:
            with open(os.path.join(self.out, 'logs',
                ...
                json.dump({'phase': self.phase,
                           ...
                           'policy_switch': {
                               'captured_pose_minimum_gain_ratio':
                                   self._fast_fixed_stand_params[
                                       '/simenv/fixed_stand_fast_minimum_gain_ratio'],
                               'fixed_stand_timeout_sec':
                                   self.policy_switch_fixed_stand_timeout,
                           },
                           'trace': self.trace},
                          stream)
