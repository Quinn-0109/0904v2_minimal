...
from devel_space_resolver import (resolve_controller_settle_seconds,
                                 resolve_junior_ctrl)
...
def main():
    ...
    # Keep the locomotion process inside this workspace.  Preference order:
    # SIMENV_DEVEL_SPACE (explicit), devel, devel_0803b (legacy fallback).
    # A candidate must be a regular executable whose realpath stays inside
    # this workspace.
    executable = resolve_junior_ctrl(workspace)
    print("Using junior_ctrl: {}".format(executable), flush=True)
    ...
    # Optional settle window before constructing the legacy controller.  The
    # unpowered A1 collapses under a wall-clock sleep while physics runs, so
    # the default is 0.0: exec immediately once controllers are live.
    settle_seconds = resolve_controller_settle_seconds()
    if settle_seconds > 0.0:
        print("Settling controllers for {:.1f}s before junior_ctrl".format(
            settle_seconds), flush=True)
