...
namespace unitree_legged_control 
{
    ...
    void UnitreeJointController::starting(const ros::Time& time)
    {
        ...
        // SIMENV_CALF_WRAP_FIX: Gazebo may expose the calf at q + 2*pi.
        // Unitree state estimation and the fixed-stand target use [-pi, pi].
        if (isCalf) {
            while (init_pos > 3.141592653589793) init_pos -= 6.283185307179586;
            while (init_pos < -3.141592653589793) init_pos += 6.283185307179586;
        }
        lastCmd.mode = PMSM;
        lastCmd.q = init_pos;
    ...
    void UnitreeJointController::update(const ros::Time& time, const ros::Duration& period)
    {
        ...
        // SIMENV_CALF_WRAP_FIX: use the representation nearest the Unitree policy range.
        if (isCalf) {
            while (currentPos > 3.141592653589793) currentPos -= 6.283185307179586;
            while (currentPos < -3.141592653589793) currentPos += 6.283185307179586;
        }
