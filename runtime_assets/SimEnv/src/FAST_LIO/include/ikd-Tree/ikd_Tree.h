...
public:
    ...
    // Stop the asynchronous subtree rebuild before dropping all nodes.  A
    // direct Build(empty) can otherwise free the tree while the rebuild
    // worker still owns Rebuild_Ptr, which aborts at the next parent swap.
    void Clear();
