...
{
    // The normal incremental APIs coordinate with multi_thread_rebuild(),
    // but a floor-lifecycle reset replaces the complete root and therefore
    // cannot leave that worker alive.  Join it first so Rebuild_Ptr and its
    // father pointer cannot outlive the nodes they identify.
    stop_thread();
    Delete_Storage_Disabled = true;
    delete_tree_nodes(&Root_Node);
    Root_Node = nullptr;
