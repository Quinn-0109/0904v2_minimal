...
State_Trotting::State_Trotting(CtrlComponents *ctrlComp)
    ...
    const char *gaitHeightValue = std::getenv("UNITREE_GAIT_HEIGHT");
    _gaitHeight = gaitHeightValue == nullptr ? 0.08 : std::atof(gaitHeightValue);
    if(!std::isfinite(_gaitHeight) || _gaitHeight < 0.03 || _gaitHeight > 0.15){
        _gaitHeight = 0.08;
    }
...
void State_Trotting::enter(){
    ...
    const char *disableAmpLog = std::getenv("UNITREE_DISABLE_AMP_LOG");
    if(disableAmpLog == nullptr || std::string(disableAmpLog) != "1"){
        ampthreadRunning = State_Trotting::RUNNING;
        amp_obs_thread = new std::thread(&State_Trotting::save_amp_obs_thread,this);
    }else{
        ampthreadRunning = State_Trotting::STOP;
        std::cout << "[INFO] AMP diagnostic logging disabled for stable mapping." << std::endl;
    }
}
