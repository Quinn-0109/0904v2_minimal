...
FSM::FSM(CtrlComponents *ctrlComp)
    :_ctrlComp(ctrlComp){
    _autoStartRl = getEnvBool("UNITREE_AUTO_START_RL", false);
    _autoHoldStand = getEnvBool("UNITREE_AUTO_HOLD_STAND", false);
...
void FSM::run(){
    ...
    if((_autoStartRl || _autoHoldStand) && _autoStartPhase < 2){
        if(_autoStartPhase == 0){
            _ctrlComp->ioInter->setFixedStand();
            _ctrlComp->lowState->userCmd = UserCommand::L2_A;
            _autoStartPhase = 1;
            _autoStartElapsed = 0.0;
            std::cout << "[INFO] Autonomous startup: entering fixed stand"
                      << (_autoHoldStand ? " (hold until /joy RL)." : ".")
                      << std::endl;
        }else if(_autoHoldStand){
            // Do not overwrite cmdPanel; wait for /joy (or keyboard) to request RL.
            if(_ctrlComp->lowState->userCmd == UserCommand::RL ||
               _ctrlComp->lowState->userCmd == UserCommand::RL_KEYBOARD){
                _autoStartPhase = 2;
                std::cout << "[INFO] Hold-stand released; RL requested." << std::endl;
            }
        }else{
            _autoStartElapsed += _ctrlComp->dt;
            if(_autoStartElapsed >= getEnvDouble("UNITREE_AUTO_STAND_SECONDS", 3.0)){
                _ctrlComp->ioInter->setRlCmdVel();
                _ctrlComp->lowState->userCmd = UserCommand::RL;
                _autoStartPhase = 2;
                std::cout << "[INFO] Autonomous startup: enabling RL /cmd_vel mode." << std::endl;
            }
        }
    }
