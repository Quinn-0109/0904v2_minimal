...
void Estimator::reset(){
    // Gazebo pose/joint reset is discontinuous.  Keeping the pre-reset body
    // velocity, foot positions and covariance makes the first trotting step
    // depend on the previous passive settling history and can tip the A1.
    _xhat.setZero();
    _u.setZero();
    _P.setIdentity();
    _P *= _largeVariance;
    ...
    _feetH.setZero();
    _feetPosGlobalKine.setZero();
    _feetVelGlobalKine.setZero();
    _vxFilter->clear();
    _vyFilter->clear();
    _vzFilter->clear();
}
