...
UserCommand joyCommandFromButtons(const std::vector<int>& buttons){
    ...
    if(buttonAt(buttons, 4)) return UserCommand::START;       // classic trotting
...
IOROS::IOROS():IOInterface(){
    ...
    for(auto &position : _raw_joint_position){
        position.store(0.0f);
    }
    _imu_received.store(false);

    // /joy can be delivered as soon as the async spinner starts.  Construct
    // the command panel first because joyCallback writes to it; doing this
    // after initRecv() leaves a startup window that dereferences nullptr when
    // an external teleop is already publishing.
...
void IOROS::sendCmd(const LowlevelCmd *lowCmd){
    for(int i(0); i < 12; ++i){
        ...
        float target = lowCmd->motorCmd[i].q;
        // gazebo_ros_control reports continuous-joint positions without
        // folding them to [-pi, pi].  Sending a canonical policy target
        // directly to a joint currently on another branch asks the effort
        // controller to rotate through one or more complete turns.  Preserve
        // the exact physical target but choose its nearest equivalent branch.
        if(_joint_state_received[i].load() && std::isfinite(target)){
            const float raw = _raw_joint_position[i].load();
            constexpr float twoPi = 6.28318530717958647692f;
            target += std::round((raw - target) / twoPi) * twoPi;
        }
...
void IOROS::updateMotorState(int index, const unitree_legged_msgs::MotorState& msg){
    ...
    _raw_joint_position[index].store(msg.q);
    // Keep estimator and policy observations independent of Gazebo's
    // arbitrary continuous-joint branch.  All A1 commanded joint ranges fit
    // inside [-pi, pi], so this preserves the physical configuration.
    _lowState.motorState[index].q = std::remainder(
        msg.q, static_cast<float>(2.0 * M_PI));
