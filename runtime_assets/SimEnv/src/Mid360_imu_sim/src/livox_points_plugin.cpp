...
void LivoxPointsPlugin::Load(gazebo::sensors::SensorPtr _parent, sdf::ElementPtr sdf) {
    ...
    std::cerr << "[livox] csv rows=" << datas.size() << std::endl;

    sdfPtr = sdf;
    if (!sdfPtr->HasElement("ray")) {
        ROS_ERROR_STREAM("Livox plugin missing <ray> element");
        return;
    }
    ...
    // Never pass "/scan" (or any absolute topic) as a ROS node name — it crashes.
    if (!ros::isInitialized()) {
        int argc = 0;
        char **argv = nullptr;
        ros::init(argc, argv, "livox_gazebo_plugin", ros::init_options::NoSigintHandler);
    }
    ...
    maxPointSize = static_cast<int64_t>(aviaInfos.size());
    if (maxPointSize <= 0) {
        ROS_ERROR_STREAM("Livox CSV produced zero scan directions");
        return;
    }

    std::cerr << "[livox] before RayPlugin::Load" << std::endl;
    RayPlugin::Load(_parent, sdfPtr);
    std::cerr << "[livox] after RayPlugin::Load" << std::endl;
    laserMsg.mutable_scan()->set_frame(_parent->ParentName());
    parentEntity = this->world->EntityByName(_parent->ParentName());
    if (!parentEntity) {
        ROS_ERROR_STREAM("Livox plugin parent entity missing: " << _parent->ParentName());
        return;
    }
    auto physics = world->Physics();
    std::cerr << "[livox] before CreateCollision" << std::endl;
    laserCollision = physics->CreateCollision("multiray", _parent->ParentName());
    std::cerr << "[livox] after CreateCollision" << std::endl;
    ...
    if (samplesStep > static_cast<int>(maxPointSize)) {
        samplesStep = static_cast<int>(maxPointSize);
    }
    ROS_INFO_STREAM("sample:" << samplesStep);
    ROS_INFO_STREAM("downsample:" << downSample);
    rayShape->RayShapes().reserve(samplesStep / downSample);
    std::cerr << "[livox] before rayShape Load/Init samples=" << samplesStep << std::endl;
    rayShape->Load(sdfPtr);
    rayShape->Init();
    std::cerr << "[livox] after rayShape Init" << std::endl;
    ...
    ROS_INFO_STREAM("Livox plugin Load complete with " << (samplesStep / downSample) << " rays");
}

void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape && parentEntity && raySensor) {
