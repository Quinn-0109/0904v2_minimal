...
class RefinerConfig:
    ...
    # A preplanned route may prefer a fresh global replan over an expensive
    # local search around a newly colliding non-start waypoint. Keep repair
    # enabled by default so existing callers retain their exact behaviour.
    repair_colliding_waypoints: bool = True
...
class PathRefiner:
    ...
    def _segment_safe(self, a: Point, b: Point,
                      check_b_first: bool = False) -> bool:
        ...
        indices = range(count, -1, -1) if check_b_first else range(count + 1)
    ...
    def _candidate(self, points: List[Point], index: int) -> Optional[Point]:
        ...
        # Search the nearest displacement ring first.  The former square scan
        # evaluated every candidate out to the full radius even after finding
        # a valid 15--21 cm repair.  Each candidate audits both adjoining
        # segments, so one stale A* waypoint could trigger several hundred
        # live voxel-service calls.  A nearest-ring repair is both the least
        # route-deforming choice and lets us stop before testing farther rings.
        ...
        for iy in range(-bound, bound + 1):
            for ix in range(-bound, bound + 1):
                ...
                if offset <= 1e-9 or offset > self.config.refinement_search_radius + 1e-9:
                    continue
                rings.setdefault(round(offset, 9), []).append((ix, iy))
        for ring_offset in sorted(rings):
            best: Optional[Tuple[float, Point]] = None
            for ix, iy in rings[ring_offset]:
                offset = math.hypot(ix * step, iy * step)
                ...
                # A replacement is needed because the original waypoint was
                # colliding. Invalid offsets therefore almost always fail
                # close to the candidate end of the incoming segment. Audit
                # the exact same samples and forward body yaw, but visit that
                # end first. This keeps the safety result identical while
                # avoiding a full 4--5 m service scan for every rejected
                # 15 cm offset (fix15 F2 room 4 used 483 service calls and
                # 15.9 wall seconds before moving).
                if (not safe or not self._segment_safe(
                        previous, candidate, check_b_first=True)):
                    continue
        ...
        for index, (point, yaw) in enumerate(zip(list(points), original_yaws)):
            ...
            if index == 0 and not self.config.repair_start_waypoint:
                return self._result(False, "start_footprint_collision", points,
                                    ...
                                    started)
            if (index > 0 and reason == "footprint_collision" and
                    not self.config.repair_colliding_waypoints):
                return self._result(
                    False, "waypoint_collision_replan_required", points,
