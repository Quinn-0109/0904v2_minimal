...
class LocalDoorwayDetector:
    def __init__(self):
        ...
        self.pose_rebase_reset_distance = float(rospy.get_param(
            "~pose_rebase_reset_distance_m", 4.0))
    ...
    def _on_odom(self, message):
        ...
        if not all(math.isfinite(v) for v in pose):
            return
        with self.lock:
            # FAST-LIO is deliberately rebased to truth after each upper-floor
            # seam.  Retaining a trajectory sample from the previous map frame
            # makes _opening_candidates infer its corridor axis from that
            # multi-metre jump, so both real side doors look like forward wall
            # gaps until the robot has already passed the near row.  A rebase
            # starts a new local-geometry epoch; candidates still have to be
            # rediscovered and confirmed from the fresh projection.
            if (self.pose is not None and
                    math.hypot(pose[0] - self.pose[0],
                               pose[1] - self.pose[1]) >=
                    max(1.0, self.pose_rebase_reset_distance)):
                self.trajectory = []
                self.tracks.clear()
