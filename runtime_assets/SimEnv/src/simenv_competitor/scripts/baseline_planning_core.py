...
def room_waypoint_force_heading(
        room_role: str, current_waypoint: Point,
        next_waypoint: Point) -> bool:
    """Require heading lock for every EXIT, including compacted one-leg exits."""
    role = str(room_role or "").upper()
    return bool(role == "EXIT" or
                (role in ("G3", "G4") and
                 tuple(next_waypoint) != tuple(current_waypoint)))


def room_waypoint_heading(
        room_role: str, current_position: Point,
        current_waypoint: Point, next_waypoint: Point) -> float:
    """Choose the physical heading for one audited room-route segment.

    EXIT is a rotate-before-translate contract.  Its old generic look-ahead
    heading faced the *next* leg while executing the current one, turning an
    L-shaped safe path into consecutive high-speed lateral motions.  run172
    fell at exactly that transition.  Face the leg currently being executed;
    when the following goal is dispatched it rotates to that following leg
    before translating, so the final leg still aligns with the outward door
    normal.  Other roles retain the established look-ahead behavior.
    """
    point = (float(current_waypoint[0]), float(current_waypoint[1]))
    current = (float(current_position[0]), float(current_position[1]))
    following = (float(next_waypoint[0]), float(next_waypoint[1]))
    if (str(room_role or "").upper() == "EXIT" and
            math.hypot(point[0]-current[0], point[1]-current[1]) > 0.05):
        return math.atan2(point[1]-current[1], point[0]-current[0])
    if following != point:
        return math.atan2(following[1]-point[1], following[0]-point[0])
    return math.atan2(point[1]-current[1], point[0]-current[0])
...
def corridor_local_door_context_allowed(
        ...
        maximum_lateral_error: float = 0.85,
        guided_entry: bool = False,
        guided_axis_progress: float = 0.0,
        guided_axis_lateral_error: float = math.inf,
        guided_minimum_progress: float = 3.5) -> bool:
    ...
    while its ordinary fallback is deliberately narrow: the entry-forward
    already be locked, and the robot must have continued forward on that
    locked line.

    A fresh upper-floor manager can additionally start at a separately
    verified corridor handoff pose with a heading expressed in its live
    odometry frame.  ``guided_entry`` admits a side opening only after
    measured forward progress on that line and a small lateral error.  The
    default is disabled, so an opening in the initial floor-1 lobby still
    cannot create the corridor/room phase.
    """
    if raw_is_corridor:
        return True
    try:
        ...
        guided_progress = float(guided_axis_progress)
        guided_lateral = float(guided_axis_lateral_error)
        guided_required = float(guided_minimum_progress)
    ...
    guided_context = bool(
        guided_entry and
        all(math.isfinite(value) for value in
            (guided_progress, guided_lateral, guided_required, tolerance)) and
        guided_required > 0.0 and tolerance > 0.0 and
        guided_progress >= guided_required and
        guided_lateral <= tolerance)
    return locked_context or guided_context
...
def room_live_start_override_allowed(
        current: Sequence[float], trajectory: Sequence[Sequence[float]],
        entry_truth_crossing_confirmed: bool, active_room: bool,
        truth_exploration_authorized: bool) -> bool:
    """Authorize removal of a stale self voxel at an entered-room pose.

    This is deliberately narrower than a path collision exception.  It only
    establishes that waypoint zero is the robot's continuous, physically
    occupied pose after a truth-confirmed doorway crossing.  Callers may
    override the current body envelope, but every sample beyond that envelope
    must still pass the ordinary live 3-D footprint service.
    """
    if (not entry_truth_crossing_confirmed or not active_room or
            not truth_exploration_authorized or len(current) < 2 or
            len(trajectory) < 2):
        return False
    try:
        point = (float(current[0]), float(current[1]))
        ...
        return False
    if (len(recent) < 2 or
            not all(math.isfinite(value) for value in point) or
            math.hypot(recent[-1][0] - point[0],
                       recent[-1][1] - point[1]) > 0.12):
        return False
    if any(not all(math.isfinite(value) for value in sample)
           for sample in recent):
        return False
    # A reset/teleport invalidates the continuity evidence.  Normal room
    # locomotion samples are far below this deliberately conservative bound.
    return not any(math.hypot(b[0] - a[0], b[1] - a[1]) > 0.75
                   for a, b in zip(recent[:-1], recent[1:]))


def truth_open_room_goal_override_allowed(
        source: str, room_role: str, diagnostic: dict,
        entry_truth_crossing_confirmed: bool, active_room: bool) -> bool:
    """Authorize a stale 2-D endpoint override for a truth-locked open view.

    The caller must independently prove that the live 3-D endpoint footprint
    is clear and re-audit the complete resulting path.  Obstacle-front room
    contracts are intentionally excluded.
    """
    if (str(source) != "lightweight_room_semantic" or
            str(room_role).upper() not in ("G3", "G4") or
            not entry_truth_crossing_confirmed or not active_room or
            not isinstance(diagnostic, dict)):
        return False
    return bool(
        diagnostic.get("truth_layout_centerline_fallback") and
        diagnostic.get("live_3d_audit_required") and
        diagnostic.get("two_pose_strategy") ==
        "truth_layout_open_centerline_deep_near" and
        diagnostic.get("two_pose_obstacle") is None)
...
def astar_safe_path(grid: OccupancyGrid2D, start: Point, goal: Point,
                    ...
                    maximum_expansions: int = 0,
                    allow_blocked_goal: bool = False) -> dict:
    ...
    if (not allow_blocked_goal and
            not _cell_clear(grid, goal_cell, clearance_radius)):
    ...
    while frontier:
        ...
        if current == goal_cell:
            ...
            reason = ("path_found_goal_override" if allow_blocked_goal else
                      "path_found_start_override" if allow_blocked_start else
                      "path_found")
            return {
                "success": True,
                "reason": reason,
        ...
        for dx, dy in neighbors:
            ...
            if clear is None:
                clear = _cell_clear(
                    ...
                    ignored_cell=(goal_cell if allow_blocked_goal else
...
def ordered_reachable_goal_path(
        grid: OccupancyGrid2D, start: Point, goals: Sequence[Point],
        clearance_radius: float = 0.30,
        reached_tolerance: float = 0.30,
        allow_blocked_start: bool = False) -> dict:
    """Select the first reachable ordered goal with one component search.

    Candidate eligibility exactly matches :func:`astar_safe_path`: observed
    free space, circular clearance, the optional blocked-start-cell override,
    and four-neighbour connectivity. After the flood fill chooses the highest
    priority reachable goal, the unchanged A* planner is called once for that
    goal so the returned reason and path, including tie-breaking, are exactly
    the same as the sequential reference implementation.
    """
    candidates = [(float(goal[0]), float(goal[1])) for goal in goals]
    if not candidates:
        return {
            "success": False, "reason": "no_candidate_goals", "path": [],
            "goal": None, "goal_index": None, "candidate_reasons": [],
            "component_expansions": 0, "astar_expansions": 0,
        }

    start_point = (float(start[0]), float(start[1]))
    tolerance = float(reached_tolerance)
    near = [
        math.hypot(goal[0] - start_point[0],
                   goal[1] - start_point[1]) <= tolerance
        for goal in candidates
    ]
    candidate_reasons: List[Optional[str]] = [None] * len(candidates)

    def selected_result(index: int, component_expansions: int) -> dict:
        astar_result = astar_safe_path(
            grid, start_point, candidates[index], clearance_radius, tolerance,
            allow_blocked_start=allow_blocked_start)
        candidate_reasons[index] = astar_result.get("reason")
        result = dict(astar_result)
        result.update({
            "goal": candidates[index], "goal_index": index,
            "candidate_reasons": candidate_reasons,
            "component_expansions": int(component_expansions),
            "astar_expansions": int(astar_result.get("expansions", 0)),
        })
        return result

    start_cell = grid.world_to_cell(start_point)
    start_failure = None
    if start_cell is None:
        start_failure = "start_outside_map"
    elif (not allow_blocked_start and not _cell_clear(
            grid, start_cell, clearance_radius, allow_unknown=True)):
        start_failure = "start_footprint_blocked"
    if start_failure is not None:
        for index, is_near in enumerate(near):
            if is_near:
                for earlier in range(index):
                    candidate_reasons[earlier] = start_failure
                return selected_result(index, 0)
        return {
            "success": False, "reason": "no_reachable_goal", "path": [],
            "goal": None, "goal_index": None,
            "candidate_reasons": [start_failure] * len(candidates),
            "component_expansions": 0, "astar_expansions": 0,
        }

    candidate_cells: List[Optional[Tuple[int, int]]] = []
    goal_clear_cache: Dict[Tuple[int, int], bool] = {}
    for index, (is_near, goal) in enumerate(zip(near, candidates)):
        if is_near:
            candidate_cells.append(None)
            candidate_reasons[index] = "already_at_goal"
            continue
        cell = grid.world_to_cell(goal)
        if cell is None:
            candidate_cells.append(None)
            candidate_reasons[index] = "goal_outside_map"
            continue
        clear = goal_clear_cache.get(cell)
        if clear is None:
            clear = _cell_clear(grid, cell, clearance_radius)
            goal_clear_cache[cell] = clear
        if not clear:
            candidate_cells.append(None)
            candidate_reasons[index] = "goal_footprint_blocked"
            continue
    ...
    first_viable = next((
        index for index, (is_near, cell) in enumerate(
            zip(near, candidate_cells))
        if is_near or cell is not None), None)
    ...
    if near[first_viable] or candidate_cells[first_viable] == start_cell:
        return selected_result(first_viable, 0)

    frontier = deque([start_cell])
    visited = {start_cell}
    clear_cache: Dict[Tuple[int, int], bool] = {}
    component_expansions = 0
    primary_cell = candidate_cells[first_viable]
    primary_reached = False
    neighbors = ((1, 0), (-1, 0), (0, 1), (0, -1))
    while frontier:
        current = frontier.popleft()
        component_expansions += 1
        if current == primary_cell:
            primary_reached = True
            break
        for dx, dy in neighbors:
            nxt = current[0] + dx, current[1] + dy
            if (nxt in visited or
                    not (0 <= nxt[0] < grid.width and
                         0 <= nxt[1] < grid.height)):
                continue
            clear = clear_cache.get(nxt)
            if clear is None:
                clear = _cell_clear(
                    grid, nxt, clearance_radius,
                    ignored_cell=(start_cell if allow_blocked_start else None))
                clear_cache[nxt] = clear
            if not clear:
                continue
            visited.add(nxt)
    ...
    selected = first_viable if primary_reached else next((
        index for index, (is_near, cell) in enumerate(
            zip(near, candidate_cells))
        if is_near or (cell is not None and cell in visited)), None)
    if selected is None:
        for index, cell in enumerate(candidate_cells):
            if cell is not None and candidate_reasons[index] is None:
                candidate_reasons[index] = "goal_unreachable"
        return {
            "success": False, "reason": "no_reachable_goal", "path": [],
            "goal": None, "goal_index": None,
            "candidate_reasons": candidate_reasons,
            "component_expansions": component_expansions,
            "astar_expansions": 0,
        }
    for index in range(selected):
        if (candidate_cells[index] is not None and
                candidate_reasons[index] is None):
            candidate_reasons[index] = "goal_unreachable"
    return selected_result(selected, component_expansions)
...
def compact_collinear_portal_anchors(
        anchors: Sequence[Point], tolerance: float = 0.18) -> List[Point]:
    """Collapse an ordered straight doorway crossing to its two endpoints.

    Occupancy-grid snapping can move the saved door centre by one 15 cm cell,
    even though the inside, centre and corridor anchors still describe the
    same physical normal ray.  Returning only the aligned inside point and
    final corridor point avoids braking at every short semantic marker.  A
    genuine bend, reversed ordering, or lateral deviation is preserved.  The
    caller remains responsible for validating the replacement chord.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) <= 2:
        return points
    ...
    for point in points:
        ...
        perpendicular = abs(px * dy - py * dx) / length
        if (projection < -1e-6 or projection > 1.0 + 1e-6 or
                projection + 1e-6 < previous_projection or
                perpendicular > allowed):
            return points
        previous_projection = projection
    return [start, finish]


def compact_piecewise_collinear_anchors(
        anchors: Sequence[Point], tolerance: float = 0.08) -> List[Point]:
    """Remove stop points inside straight runs while retaining every bend.

    Unlike :func:`compact_collinear_portal_anchors`, this helper also handles
    an L-shaped semantic route.  It never replaces a bend: a middle point is
    removed only when it lies between its neighbours, in order, and within
    ``tolerance`` of their chord.  The caller still performs the normal dense
    collision audit on every retained chord before execution.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) <= 2:
        return points
    compacted: List[Point] = [points[0]]
    for index in range(1, len(points) - 1):
        ...
        length_sq = dx * dx + dy * dy
        if length_sq <= 1e-8:
            compacted.append(middle)
            continue
        length = math.sqrt(length_sq)
        mx, my = middle[0] - start[0], middle[1] - start[1]
        projection = (mx * dx + my * dy) / length_sq
        perpendicular = abs(mx * dy - my * dx) / length
        if (projection < -1e-6 or projection > 1.0 + 1e-6 or
                perpendicular > max(0.0, float(tolerance))):
            compacted.append(middle)
    compacted.append(points[-1])
    return compacted


def portal_door_center_anchor_index(
        anchors: Sequence[Point], corridor_extension_applied: bool
        ) -> Optional[int]:
    """Return the semantic door-centre index in an EXIT anchor sequence.

    A normal portal contributes ``inside -> centre -> corridor``.  The room
    scheduler may append one verified corridor-clearance endpoint so executor
    tolerance cannot stop on the door plane; in that case the centre moves
    from ``-2`` to ``-3``.  Keep this convention in one place so callers do
    not mistake the corridor-side anchor for the door centre.
    """
    count = len(anchors)
    index = count - (3 if corridor_extension_applied else 2)
    return index if 0 <= index < count else None


def passed_collinear_portal_prefix(
        current: Point, anchors: Sequence[Point],
        lateral_tolerance: float = 0.22,
        current_lateral_tolerance: float = 0.50,
        minimum_progress: float = 0.20,
        minimum_remaining: float = 0.25) -> Optional[dict]:
    """Describe a safe-to-audit direct suffix after a passed portal prefix.

    A mirrored paired-room entry is scheduled immediately after leaving the
    opposite room.  The robot can consequently already be between the saved
    corridor-side anchor and the door centre.  Replaying the complete anchor
    list first sends it backwards before it crosses the same portal again.

    This helper makes only the geometric decision: all anchors must form one
    ordered portal ray and the live pose must lie near, and strictly inside,
    that ray.  Anchors ahead of the live projection (normally door centre and
    inside) remain mandatory, so a laterally displaced corridor pose cannot
    cut diagonally through the frame.  The caller must still collision-check
    every returned chord using the live 3-D footprint service.  A real bend,
    a distant off-axis pose, or a pose before/after the portal returns
    ``None``.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) < 3:
    ...
    compacted = compact_collinear_portal_anchors(
        points, tolerance=lateral_tolerance)
    if len(compacted) != 2:
        return None
    start, finish = compacted
    dx, dy = finish[0] - start[0], finish[1] - start[1]
    length = math.hypot(dx, dy)
    if length <= 1e-8:
        return None
    px = float(current[0]) - start[0]
    py = float(current[1]) - start[1]
    progress = (px * dx + py * dy) / length
    lateral_error = abs(px * dy - py * dx) / length
    remaining = length - progress
    if (progress < max(0.0, float(minimum_progress)) or
            remaining < max(0.0, float(minimum_remaining)) or
            lateral_error > max(
                0.0, float(current_lateral_tolerance))):
        return None
    projections = [
        ((point[0] - start[0]) * dx +
         (point[1] - start[1]) * dy) / length
        for point in points]
    skipped = sum(1 for value in projections[:-1]
                  if value <= progress + 1e-6)
    if skipped < 1:
        return None
    live = (float(current[0]), float(current[1]))
    suffix = points[skipped:]
    if not suffix:
        return None
    path = [live] + suffix
    return {
        ...
        "original_anchor_count": len(points),
    }


def portal_entry_early_turn_candidates(
        current: Point, anchors: Sequence[Point],
        turn_profiles: Sequence[Tuple[float, float]] = (
            (0.55, 0.35), (0.70, 0.30), (0.45, 0.45),
        )) -> List[List[Point]]:
    """Build portal-entry paths that rotate before the body reaches a jamb.

    Anchors are ordered corridor-side -> door-centre -> room-side. When
    the robot approaches laterally along a corridor, waiting until the first
    anchor to turn can sweep its forward collision cylinder into the near
    jamb even though the centreline itself is free. Each candidate inserts
    one bend both tangentially before and normally outside the corridor-side
    anchor. This helper is geometric only; callers must accept a candidate
    only after a dense live 3-D footprint audit leaves every point unchanged.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) < 3:
        return []
    corridor_anchor, door_center = points[0], points[1]
    nx = door_center[0] - corridor_anchor[0]
    ny = door_center[1] - corridor_anchor[1]
    normal_length = math.hypot(nx, ny)
    if normal_length <= 1e-8:
        return []
    nx, ny = nx / normal_length, ny / normal_length
    approach_x = corridor_anchor[0] - float(current[0])
    approach_y = corridor_anchor[1] - float(current[1])
    normal_progress = approach_x * nx + approach_y * ny
    tangent_x = approach_x - normal_progress * nx
    tangent_y = approach_y - normal_progress * ny
    tangent_length = math.hypot(tangent_x, tangent_y)
    if tangent_length < 0.65:
        return []
    tangent_x, tangent_y = tangent_x / tangent_length, tangent_y / tangent_length
    live = (float(current[0]), float(current[1]))
    candidates = []
    for tangent_retreat, outward_offset in turn_profiles:
        retreat = max(0.20, float(tangent_retreat))
        outward = max(0.15, float(outward_offset))
        if retreat >= tangent_length - 0.15:
            continue
        bend = (
            corridor_anchor[0] - tangent_x * retreat - nx * outward,
            corridor_anchor[1] - tangent_y * retreat - ny * outward,
        )
        if (math.hypot(bend[0] - live[0], bend[1] - live[1]) < 0.15 or
                math.hypot(corridor_anchor[0] - bend[0],
                           corridor_anchor[1] - bend[1]) < 0.25):
            continue
        candidates.append([live, bend] + points)
    return candidates


def portal_entry_live_corridor_anchor_skip_candidates(
        current: Point, anchors: Sequence[Point]) -> List[List[Point]]:
    """Build fully auditable entries after the corridor anchor is redundant.

    The detector's corridor-side anchor can sit one body radius from a jamb.
    If the robot is already farther outside that door plane, braking and
    rotating at the saved anchor is both unnecessary and less safe.  Keep the
    semantic door centre and inside anchor, and offer direct/early-turn paths
    whose complete geometry must still pass the caller's dense live 3-D audit
    unchanged.  This helper never declares a path safe by itself.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) != 3:
        return []
    corridor_anchor, door_center, inside_anchor = points
    nx = door_center[0] - corridor_anchor[0]
    ny = door_center[1] - corridor_anchor[1]
    normal_length = math.hypot(nx, ny)
    if normal_length <= 1e-8:
    ...
    nx, ny = nx / normal_length, ny / normal_length
    live = (float(current[0]), float(current[1]))
    # Never skip an anchor after the body has already crossed the door plane.
    outside_progress = ((live[0] - corridor_anchor[0]) * nx +
                        (live[1] - corridor_anchor[1]) * ny)
    if outside_progress > 0.10:
        return []
    if math.hypot(live[0] - door_center[0],
                  live[1] - door_center[1]) > 3.0:
        return []
    candidates = [[live, door_center, inside_anchor]]
    for early in portal_entry_early_turn_candidates(live, points):
        # early = live -> bend -> corridor -> centre -> inside.
        candidates.append([early[0], early[1], early[3], early[4]])
    return candidates


def preflight_entry_inside_anchor_replacement_allowed(
        anchors: Sequence[Point], refined_path: Sequence[Point],
        anchor_tolerance: float = 0.12, maximum_shortening: float = 0.50,
        minimum_inside_depth: float = 0.45,
        maximum_lateral_error: float = 0.30,
        maximum_extension: float = 1.00) -> bool:
    """Validate a bounded, still-inside repair of an ENTRY endpoint.

    ``anchors`` must be corridor-side -> door-centre -> room-side.  The first
    two semantic portal anchors remain immutable.  Only the final room-side
    endpoint may move within a narrow continuation of the portal ray while it
    stays far enough beyond the door plane to prove a physical crossing.  A
    bounded deeper/lateral replacement is useful when the saved shallow point
    becomes occupied after the room map fills in.
    Collision safety is deliberately outside this geometric helper: callers
    may use it only after the complete refined path has passed their live 3-D
    footprint audit.
    """
    if len(anchors) != 3 or not refined_path:
        return False
    points = [(float(point[0]), float(point[1])) for point in anchors]
    refined = [(float(point[0]), float(point[1]))
               for point in refined_path]
    corridor_anchor, door_center, inside_anchor = points
    if not all(any(math.hypot(point[0] - anchor[0],
                              point[1] - anchor[1]) <= anchor_tolerance
                       for point in refined)
               for anchor in (corridor_anchor, door_center)):
        return False
    nx = inside_anchor[0] - door_center[0]
    ny = inside_anchor[1] - door_center[1]
    inside_depth = math.hypot(nx, ny)
    if inside_depth <= minimum_inside_depth:
        return False
    nx, ny = nx / inside_depth, ny / inside_depth
    for point in refined:
        dx = point[0] - door_center[0]
        dy = point[1] - door_center[1]
        depth = dx * nx + dy * ny
        lateral = abs(dx * ny - dy * nx)
        axial_shift = depth - inside_depth
        if (depth >= minimum_inside_depth and
                axial_shift >= -maximum_shortening - 1e-6 and
                axial_shift <= maximum_extension + 1e-6 and
                lateral <= maximum_lateral_error and
                math.hypot(point[0] - inside_anchor[0],
                           point[1] - inside_anchor[1]) <=
                max(maximum_shortening, maximum_extension) + 1e-6):
            return True
    return False


def portal_entry_inside_anchor_candidates(
        anchors: Sequence[Point], minimum_inside_depth: float = 0.55,
        maximum_extension: float = 1.00,
        lateral_offsets: Sequence[float] =
        (0.0, 0.15, -0.15, 0.28, -0.28)) -> List[Point]:
    """Return bounded deeper ENTRY endpoints for a fresh live-3D audit.

    This function proposes geometry only.  The caller must retain the
    corridor and door-centre anchors and accept a candidate only when the
    complete path passes collision checking unchanged.
    """
    if len(anchors) != 3:
        return []
    _, door_center, inside_anchor = [
        (float(point[0]), float(point[1])) for point in anchors]
    nx = inside_anchor[0] - door_center[0]
    ny = inside_anchor[1] - door_center[1]
    inside_depth = math.hypot(nx, ny)
    if inside_depth <= 1e-6:
        return []
    nx, ny = nx / inside_depth, ny / inside_depth
    tx, ty = -ny, nx
    upper_depth = inside_depth + max(0.0, float(maximum_extension))
    requested_depths = (
        max(float(minimum_inside_depth), inside_depth + 0.15),
        max(0.85, inside_depth + 0.30),
        max(1.10, inside_depth + 0.50),
        max(1.35, inside_depth + 0.75),
    )
    depths = []
    for depth in requested_depths:
        depth = min(float(depth), upper_depth)
        if (depth <= inside_depth + 0.05 or
                any(abs(depth - saved) <= 1e-6 for saved in depths)):
            continue
        depths.append(depth)
    candidates = []
    for depth in depths:
        for lateral in lateral_offsets:
            ...
            candidates.append((
                door_center[0] + nx * depth + tx * lateral,
                door_center[1] + ny * depth + ty * lateral))
    return candidates


def portal_entry_parallel_lane_candidates(
        anchors: Sequence[Point], door_width: float,
        body_edge_margin: float = 0.40,
        requested_offsets: Sequence[float] =
        (0.18, -0.18, 0.28, -0.28)) -> List[Tuple[float, List[Point]]]:
    """Build bounded parallel portal rays inside the measured aperture.

    A rolling 2-D projection can mark the nominal door-centre-to-inside
    endpoint occupied even though another part of a wide doorway is clear.
    Move *all* semantic anchors by the same tangent offset so every candidate
    still crosses the wall plane normally.  This helper only proposes
    geometry; callers must run normal A* and live 3-D footprint checks.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) != 3:
        return []
    ...
    tx, ty = -ny, nx
    maximum_offset = max(
        0.0, 0.5 * float(door_width) - max(0.0, float(body_edge_margin)))
    candidates = []
    saved_offsets = []
    for requested in requested_offsets:
        requested = float(requested)
        if abs(requested) < 0.05 or maximum_offset < 0.05:
            continue
        offset = math.copysign(min(abs(requested), maximum_offset), requested)
        if any(abs(offset - saved) <= 1e-6 for saved in saved_offsets):
            continue
        saved_offsets.append(offset)
        shifted = [(point[0] + tx * offset, point[1] + ty * offset)
                   for point in points]
        candidates.append((offset, shifted))
    return candidates


def portal_entry_aperture_lane_candidates(
        anchors: Sequence[Point], door_width: float,
        body_edge_margin: float = 0.40,
        requested_offsets: Sequence[float] =
        (0.18, -0.18, 0.28, -0.28),
        inside_extensions: Sequence[float] =
        (0.0, 0.25, 0.50, 0.75)) -> List[Tuple[float, float, List[Point]]]:
    """Keep corridor staging fixed while crossing a clear aperture lane.

    Unlike a fully parallel ray, this bounded family does not move a known
    reachable corridor-side anchor toward a jamb.  It shifts the wall-plane
    and room-side anchors together inside the measured opening, and may
    extend only the final anchor farther into the room.  Safety remains the
    caller's responsibility through segmented A* and live 3-D auditing.
    """
    points = [(float(point[0]), float(point[1])) for point in anchors]
    if len(points) != 3:
        return []
    ...
    bounded_offsets = [0.0]
    for requested in requested_offsets:
        if maximum_offset < 0.05:
            break
        offset = math.copysign(
            min(abs(float(requested)), maximum_offset), float(requested))
        if not any(abs(offset - saved) <= 1e-6
                   for saved in bounded_offsets):
            bounded_offsets.append(offset)
    candidates = []
    for extension in inside_extensions:
        extension = max(0.0, min(0.85, float(extension)))
        for offset in bounded_offsets:
            if abs(offset) <= 1e-8 and extension <= 1e-8:
                continue
            shifted_center = (
                door_center[0] + tx * offset,
                door_center[1] + ty * offset)
            shifted_inside = (
                inside_anchor[0] + tx * offset + nx * extension,
                inside_anchor[1] + ty * offset + ny * extension)
            candidates.append((offset, extension, [
                corridor_anchor, shifted_center, shifted_inside]))
    return candidates


def two_pose_edge_hint_priority_allowed(
        completed_views: int, left_edge_frames: int,
        right_edge_frames: int, strict_hits: int) -> bool:
    """Use an accumulated image-edge hint only at the first physical view.

    Edge counters are room cumulative. Reusing a pre-G3 hint at G4 can force
    both 170-degree fans in the same direction, especially when the hint came
    from a red-box distractor. The second translated view must instead use its
    fresh inward fan geometry so it covers the complementary room sector.
    """
    return bool(
        int(completed_views) == 0 and
        int(left_edge_frames) + int(right_edge_frames) > 0 and
        int(strict_hits) < 3)
...
class CorridorBranchScheduler:
    ...
    def merge_ids(self, keep_id: str,
        ...
        if keep.side != duplicate.side:
            return keep
        # A connected free-space component can span the corridor and two
        # physically distinct rooms. Never let that coarse component label
        # overwrite a completed door with a farther same-side portal. The
        # caller must continue with the newly registered physical branch when
        # neither station nor entry target satisfies the geometric identity
        # gate.
        if not self._same_region(keep, duplicate):
            return duplicate
